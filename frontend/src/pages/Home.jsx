...
const getSubjectsList = (mode) => [
  {
    ...
    icon: <CalculateIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#90caf9' : '#1565c0' }} />,
  ...
  },
  {
    name: 'Lengua',
    path: '/juegos/lengua',
    icon: <MenuBookIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#ffcc80' : '#e65100' }} />,
  ...
  },
  {
    ...
    icon: <PublicIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#a5d6a7' : '#2e7d32' }} />,
  ...
  },
  {
    ...
    icon: <BiotechIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#f48fb1' : '#c2185b' }} />,
  ...
  },
  {
    name: 'Inglés',
    path: '/juegos/ingles',
    icon: <TranslateIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#ce93d8' : '#6a1b9a' }} />,
  ...
  },
  {
    name: 'Música',
    path: '/juegos/musica',
    icon: <MusicNoteIcon sx={{ fontSize: 38, color: mode === 'dark' ? '#fff59d' : '#f57f17' }} />,
  ...
  return (
    ...
    >
      ...
      <Box className="home-hero">
        ...
        {/* Cartelito arriba de la imagen */}
        <Box className="hero-badge" sx={{ zIndex: 3 }}>
          ¡Explora y Diviértete!
        </Box>
        
        {/* IMAGEN DEL LOGO Y TÍTULO SABIKIDS */}
        <Box sx={{ width: '100%', maxWidth: '750px', margin: '0 auto', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <img 
            src="/inicio/fondo_inicio.png" 
            alt="Mascotas Sabikids" 
            className={`hero-image ${mode === 'dark' ? 'dark-mode-image' : 'light-mode-image'}`}
          />
          
          {/* TÍTULO SABIKIDS CURVADO */}
          <Box className="sabikids-curve-container">
            <svg viewBox="0 0 500 130" width="100%" height="100%" style={{ overflow: 'visible' }}>
              {/* Ruta invisible que dibuja el arco */}
              <path id="sabikids-arc" d="M 20,110 Q 250,10 480,110" fill="transparent" />
              {/* Texto que sigue la ruta */}
              <text className="sabikids-svg-text" fill={mode === 'dark' ? '#ffffff' : '#111111'}>
                <textPath href="#sabikids-arc" startOffset="50%" textAnchor="middle">
                  SABIKIDS
                </textPath>
              </text>
            </svg>
        ...
        {/* Difuminado sutil en la parte inferior */}
        <Box 
          sx={{
            position: 'absolute',
            bottom: 0,
      ...
      <Container className="subjects-section" sx={{ py: 8, maxWidth: '1400px !important' }}>
        <Box className="section-header" sx={{ textAlign: 'center', mb: 6 }}>
          <Typography variant="h2" className="section-title" sx={{ color: mode === 'dark' ? '#90caf9' : '#1565c0', mb: 1 }}>
          ...
          {subjectsList.map((subject) => (
            ...
            >
              <CardActionArea
                ...
                sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', height: '100%' }}
              >
                <Box className="subject-icon-box" sx={{ width: 56, height: 56, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255, 255, 255, 0.85)', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>
                  {subject.icon}
                </Box>
                ...
                <Typography
                  ...
                  sx={{ color: subject.textColor, fontSize: '1.05rem', fontWeight: 900, textAlign: 'center', my: 0.5 }}
                >
                  {subject.name}
                </Typography>

                <Box className="subject-badge-play" sx={{ fontSize: '0.75rem', py: 0.4, px: 1.8, borderRadius: '20px', background: 'rgba(255, 255, 255, 0.9)', fontWeight: 800, color: '#333' }}>
                  ▶ Jugar
                </Box>
              </CardActionArea>
            </Card>
      ...
      <Box className="features-section" sx={{ backgroundColor: theme.palette.background.default, py: 6, mb: 4 }}>
        <Container maxWidth="md">
          <Grid container spacing={4} justifyContent="center">
            {features.map((feat, index) => (
              <Grid item xs={12} sm={index === 2 ? 12 : 6} md={index === 2 ? 6 : 6} key={feat.title} sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box className="feature-box" sx={{ backgroundColor: feat.bg, p: 3, borderRadius: 4, width: '100%', maxWidth: '380px' }}>
